...
namespace Carbone.Bindings
{
	internal sealed class Binder : IDisposable
	{
		...
		internal bool TryResolveType(in InjectContext context, [NotNullWhen(true)] out object? instance)
		{
#if DEBUG
			if (_bindingType != null)
			{
		...
		private bool TryGetTypeProvider(Type type, object? id, [NotNullWhen(true)] out IBindingProvider? provider)
		{
			if (id == null)
			{
		...
		{
			bool TryProvide(in InjectContext context, [NotNullWhen(true)] out object? instance);
		}
		...
		private sealed class PermanentProvider : IBindingProvider
		{
			...
			public bool TryProvide(in InjectContext context, [NotNullWhen(true)] out object? instance)
			{
