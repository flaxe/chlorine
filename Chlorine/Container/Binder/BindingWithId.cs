namespace Chlorine
{
			...
			where TContract : class
	{
		...
		internal BindingWithId(Binder binder, Container container, object id)
		{
			_binder = binder;
		...
		{
			return new BindingWithProvider<TContract>(_binder, _id, new ConcreteProvider<TConcrete, TContract>(_container));
		}

		public BindingWithProvider<TContract> FromFactory<TFactory>() where TFactory : class, IFactory<TContract>
		{
			return new BindingWithProvider<TContract>(_binder, _id, new FromFactoryProvider<TFactory, TContract>(_container));
		}
